...
const SEMANTIC_GROUPS: SemanticGroup[] = [
  ...
  },
  {
    ...
    tokens: [
      ...
      { name: 'Avatar 1 Subtle',   var: '--ds-color-text-avatar1-subtle' },
      { name: 'Avatar 2 Subtle',   var: '--ds-color-text-avatar2-subtle' },
      { name: 'Avatar 3 Subtle',   var: '--ds-color-text-avatar3-subtle' },
      { name: 'Avatar 4 Subtle',   var: '--ds-color-text-avatar4-subtle' },
      { name: 'Avatar 5',          var: '--ds-color-text-avatar5' },
      { name: 'Avatar 5 Subtle',   var: '--ds-color-text-avatar5-subtle' },
    ],
  },
  {
    title: 'Fill',
  ...
  },
  {
    ...
    tokens: [
      ...
      { name: 'Brand Subtle', var: '--ds-color-border-brand-subtle' },
...
//
// These diagrams used to render Figma-exported PNGs for every icon, token
// marker, and connector arrow. Those asset URLs are generated by the Figma
// MCP server and expire ~7 days after generation, so anything committed to
// source went permanently dead. Rebuilt below using the DS's own Icon font,
// plain CSS, and the existing image-free TokenChip — numbered callouts on the
// component map to a legend instead of drawn arrows, so there's no pixel-math
// tied to a specific export to keep in sync.
  ...
  return (
    <span
      style={{
        ...
        borderRadius: '50%',
        background: 'var(--ds-color-surface-l0)',
        border: '1.5px solid var(--ds-color-border-brand)',
        color: 'var(--ds-color-text-brand)',
        fontSize: 11,
        fontWeight: 700,
        fontFamily: "'Source Code Pro', monospace",
...
function AnnotationLegend({ items }: { items: { n: number; label: string; cssVar: string }[] }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {items.map(item => (
        <div key={item.n} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <NumberBadge n={item.n} style={{ position: 'static' }} />
          <TokenChip label={item.label} cssVar={item.cssVar} />
        </div>
      ))}
    </div>
  );
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 4, height: 24, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Icon name="check" size="xs" color="var(--ds-color-icon-inverse)" aria-hidden />
              <span style={{
                fontFamily: "'Work Sans', sans-serif",
                fontWeight: 700,
                fontSize: 16,
          ...
          <NumberBadge n={1} style={{ bottom: -10, right: -10 }} />
          <NumberBadge n={2} style={{ top: -10, left: '50%', transform: 'translateX(-50%)' }} />
          <NumberBadge n={3} style={{ bottom: -10, left: 8 }} />
        </div>
        <AnnotationLegend items={[
          { n: 1, label: 'Fill Brand',            cssVar: '--ds-color-fill-brand' },
          { n: 2, label: 'Text Primary Inverse',  cssVar: '--ds-color-text-primary-inverse' },
          { n: 3, label: 'Icon Inverse',          cssVar: '--ds-color-icon-inverse' },
        ]} />
      </div>
    </div>
  );
  ...
  return (
    <span
      style={{
        ...
        animation: 'ds-docs-cursor-blink 1s step-end infinite',
        flexShrink: 0,
      }}
    />
  );
...
function InputAnnotated() {
  return (
    <div style={DIAGRAM_WRAP}>
      <style>{'@keyframes ds-docs-cursor-blink { 50% { opacity: 0; } }'}</style>
      <div style={{ display: 'flex', gap: 32, alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flexShrink: 0, width: 400 }}>
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              ...
              <Icon name="help" size="xxs" color="var(--ds-color-icon-subtle)" aria-hidden />
            </div>
            {/* Input container */}
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', flex: '1 0 0', gap: 4, alignItems: 'center', minWidth: 1, overflow: 'hidden' }}>
                    <Icon name="person" size="xs" color="var(--ds-color-icon-subtle)" aria-hidden />
                    <p style={{
                      fontFamily: 'var(--ds-body-l-regular-font-family)',
                      fontWeight: 400,
                  ...
                  <Icon name="visibility" size="xs" color="var(--ds-color-icon-subtle)" aria-hidden />
                </div>
                {/* Trailing button */}
                <div style={{ display: 'flex', height: 48, alignItems: 'center', flexShrink: 0 }}>
                  <div style={{ width: 1, height: 24, background: 'var(--ds-color-border-subtle)', flexShrink: 0 }} />
                  <div style={{ display: 'flex', height: '100%', alignItems: 'center', justifyContent: 'center', padding: '4px 16px', borderRadius: 8, flexShrink: 0 }}>
                    <Icon name="close" size="xs" color="var(--ds-color-icon-subtle)" aria-hidden />
                  </div>
                </div>
              </div>
            </div>
          ...
          <NumberBadge n={1} style={{ top: 32, right: -10 }} />
          <NumberBadge n={2} style={{ top: 32, left: 8 }} />
          <NumberBadge n={3} style={{ top: 32, right: 48 }} />
          <NumberBadge n={4} style={{ bottom: -10, left: 60 }} />
          <NumberBadge n={5} style={{ bottom: -10, left: 8 }} />
        </div>
        <AnnotationLegend items={[
          { n: 1, label: 'Fill Primary',     cssVar: '--ds-color-fill-primary' },
          { n: 2, label: 'Border Selected',  cssVar: '--ds-color-border-selected' },
          { n: 3, label: 'Icon Subtle',      cssVar: '--ds-color-icon-subtle' },
          { n: 4, label: 'Text Secondary',   cssVar: '--ds-color-text-secondary' },
          { n: 5, label: 'Text Primary',     cssVar: '--ds-color-text-primary' },
        ]} />
      </div>
    </div>
  );
